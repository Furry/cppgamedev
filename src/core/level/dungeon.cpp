...
//Class Dungeon inherits from Level.h
...
class Dungeon : public Level { 
    ...
    public: 
        ...
        HellKeeper hellKeeper;

        Dungeon(int seed, Player *player) : player(*player), hellKeeper(hellKeeper) {
            this->seed = seed;
        ...
        void renderMap(sf::Sprite sprite, sf::RenderWindow* window, TextureManager m) {
            ...
            this->hellKeeper = HellKeeper(&m);

            float x = 200;
            float y = 200;
            sf::Vector2f keeperPos = sf::Vector2f(x, y);
            this->hellKeeper.setPosition( keeperPos );
        }
        ...
        void update(int tick){
            ...
            this->hellKeeper.updatePlayer( this->player.getPosition(), this->player.pts );
        }
            ...
            if(tick > 300) {
                while( enemies.size() < 2) {
                    ...
                    switch (enemySelection) {
                        case 1: {
                            Snake *s = new Snake(&m, &this->player);
                            s->setPosition(enemyPos);
                            s->render(window);
                            addEnemies(s);
                            break; 
                        }
